...
class distribute(object):
    ...
    def issued(self):
        ...
        cmd = 'scp -P %s -r remote_install/automysql/ %s@%s:/tmp/' %\
 (self.sshport, self.username, self.ip)
        ...
        cmd1 = 'scp -P %s ./mysql.cfg %s@%s:/tmp/automysql/' %\
 (self.sshport, self.username, self.ip)
...
def main():
    ...
    hostusername = prase_config('mysql', 'my_hostusername')
    hostpasswd = prase_config('mysql', 'my_hostpassword')
    hostsshport = prase_config('mysql', 'my_hostsshport')
    ...
    for eachip in iplist:
        eachip = eachip.strip()
        D = distribute(eachip, hostusername, hostpasswd, hostsshport)
        D.issued()
        tempdata = {'ip': eachip }
        beanstalk.put(
                       json.dumps(tempdata),
